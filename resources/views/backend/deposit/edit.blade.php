...
<section class="content">
  ...
  <div class="card">
    <div class="container-fluid">
      <div class="card-body">
        <!-- <br> -->
        <form role="form" id="forms"  method="post" action="{{ route('deposit.update', $depositEdit->id) }}">
          {{ csrf_field() }}
          <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <select class="form-control select2" id="branch_id" name="branch_id" onchange="getBank(this.value)">
                ...
                @foreach($branches as $branch)
                  <option value="{{ $branch->id }}" @if($depositEdit->branch_id == $branch->id) selected @endif>{{ $branch->branch_number }}</option>
                @endforeach
              </select>
            </div>
          </div>
            ...
            <div class="form-group">
              ...
              <select class="form-control select2" id="client_id" name="client_id" onchange="getExchangeName(this.value)">
                ...
                @foreach($clients as $client)
                  <option value="{{ $client->id }}" @if($depositEdit->client_id == $client->id) selected @endif>{{ $client->name }}</option>
                @endforeach
              </select>
            </div>
          </div>
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <input type="hidden" name="exchange_id" class="form-control" id="exchange_id" value="{{ $depositEdit->exchange_id }}">
            </div>
          </div>
          <!--<div class="col-md-6">-->
          <!--  <div class="form-group">-->
          ...
          <!--      <span class="requiredAstrik">*</span>-->
          <!--    </label>-->
              <input type="hidden" name="win_amt" class="form-control" id="win_amt" placeholder="Enter Win Amount" value="{{ $depositEdit->win_amt }}">
          <!--  </div>-->
          <!--</div>-->
          <div class="col-md-6">
            <div class="form-group">
              ...
              <input type="number" name="amount" class="form-control" id="amount" placeholder="Enter Amount" value="{{ $depositEdit->amount }}">
            </div>
          </div>
        <!-- </div> -->
        <!-- <div class="row"> -->
          <div class="col-md-6">
            <div class="form-group">
              <label for="date">Date
                ...
                <input type="text" name="date" id="date" class="form-control" value="{{ date('d-m-Y h:m:s A',strtotime($depositEdit->date))}}" readonly>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              ...
              <select class="form-control select2" id="bank_id" name="bank_id">
                ...
                @foreach($banks as $bank)
                  <option value="{{ $bank->id }}" @if($depositEdit->bank_id == $bank->id) selected @endif>{{ $bank->name }}</option>
                @endforeach
              </select>
          ...
          <div class="col-md-6">
            <div class="form-group">
              <label for="date">UTR
                ...
                <input type="text" name="utr" id="utr" class="form-control" value="{{ $depositEdit->utr }}" required>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              ...
              <select class="form-control select2" id="bonus" name="bonus">
                ...
                @foreach($percentages as $percentage)
                  <option value="{{ $percentage->key }}" @if($depositEdit->bonus_percent == $percentage->key) selected @endif>{{ $percentage->value }}</option>
                @endforeach
              </select>
            ...
            <div class="form-group">
              ...
              <select class="form-control select2" id="mode_id" name="mode_id">
                ...
                @foreach($modes as $mode)
                  <option value="{{ $mode->id }}" @if($depositEdit->mode_id == $mode->id) selected @endif>{{ $mode->name }}</option>
                @endforeach
              </select>
            </div>
          </div>
        ...
        <!-- <div class="row"> -->
          <div class="col-md-6">
            <div class="form-group">
              ...
              <input type="text" name="remark" class="form-control" id="remark" placeholder="Enter Remark" value="{{ $depositEdit->remark }}">
            </div>
          </div>
        </div>
          @if(session('error'))
...
<script type="text/javascript">
  ...
  jQuery(document).ready(function() {
    // getExchangeName({{ $depositEdit->client_id }});
    //Date picker
